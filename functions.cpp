...
int parseOrderingFile(char* orderingFilename,std::vector<cog*> *accessoryLoci,struct parms *sp) {
    ...
    if (sp->het_mode == "l") { // logistic
        double max_value = 1 - 1 / (1 + exp(-1 * sp->decayRate * (float(accessoryLoci->size()) - (1 - sp->selectedProp) * float(accessoryLoci->size()))));
        for (cit = accessoryLoci->begin(), accessoryLoci->end(); cit != accessoryLoci->end(); ++cit) {
            double relative_weight_from_order = 1 - 1 / (1 + exp(-1 * sp->decayRate * (float(cindex) - (1 - sp->selectedProp) * float(accessoryLoci->size())))) * 1 / max_value;
