...
plot_time_series<-function(time_series_prefixes) {
  # function
  time_samples<-seq(from=1,to=60000,by=250)
  timeseries_list<-list()
  
  for (i in 1:6) {
    # load
    timeseries.df<-read.table(file = paste0(time_series_prefixes[i],'.1.sc.out'),
                              header = TRUE)
    # process
    timeseries.df %<>%
      dplyr::rename(Generation = SC) %>%
      dplyr::filter(Generation %in% time_samples) %>%
      tidyr::pivot_longer(cols = -Generation, names_to = "Strain_VT", values_to = "Count") %>%
      tidyr::separate(Strain_VT, c('Strain','VT'), sep = '_') %>%
      dplyr::group_by(Generation) %>%
      dplyr::mutate(N_t = sum(Count)) %>%
      dplyr::ungroup() %>%
      dplyr::group_by(Strain,Generation) %>%
      dplyr::mutate(TotalCount = sum(Count)) %>%
      dplyr::mutate(Frequency = TotalCount/N_t) %>%
      dplyr::ungroup() %>%
      dplyr::select(-VT) %>%
      dplyr::distinct() %>%
      dplyr::mutate(Recombination = recs[i]) %>%
      dplyr::mutate(Selection = selects[i])
    # store
    timeseries_list[[summaries[i]]]<-timeseries.df
  }
  
  combined_timeseries.df<-dplyr::bind_rows(timeseries_list)
  combined_timeseries.df<-relevel_df(combined_timeseries.df)
  
  time_series_plot<-
    ggplot(combined_timeseries.df, aes(x = Generation, y = Frequency, colour = Strain)) +
    geom_line(alpha = 0.5) +
    theme_minimal() +
...
get_strain_frequencies<-function(dists,slope=strain_divide_slope,intercept=strain_divide_intercept) {
  ...
  within_strain<-(dists$Distances<(intercept+slope*dists$SNPDistances))
  edge_data<-dists[within_strain,1:2]
...
make_nj_tree<-function(i) {
  ...
  if (class(i)=="data.frame") {
    ...
    # Make tree
    gdists<-rdist(s,metric="hamming")
    if (max(gdists)-min(gdists) > 0) {
      gtree<-nj(gdists)  
    }
  ...
  # do not return anything if population is monomorphic
  if (!is.null(gtree)) {
...
read_matrices<-function(prefix, r, real = NULL, snp.calc.proc=FALSE) {
  ...
  if (snp.calc.proc==TRUE) {
    ...
    tree.stats<-tree.stats[!is.na(tree.stats)]
...
relevel_df<-function(df) {
  ...
  #extra_select_levels<-as.character(unique(df[,s])[!(unique(df[,s]) %in% select_levels)])
  #extra_select_levels<-setdiff(df[,s] %>% dplyr::distinct() %>% dplyr::pull(),select_levels)
...
process_simulation_data<-function(prefixes=NULL,recs=NULL,selects=NULL,summaries=NULL,eq.loci=NULL,real.data=NULL,snp.loci=NULL,snp.calc=FALSE,strain_slope=0,strain_intercept=0,tip.anno=ma.tip.labels,real_strains=ma.strain.freqs,tree_data=ma.tree.stats) {
  ...
  if (snp.calc==TRUE) {
    ...
    strain.freq.df<-raw.strain.freq.df %>%
      ...
      dplyr::summarize(Frequency = mean(Frequencies, na.rm = TRUE), MinFrequency = min(Frequencies, na.rm = TRUE), MaxFrequency = max(Frequencies, na.rm = TRUE), .groups = "drop_last") %>%
  ...
  if (snp.calc==TRUE) {
    ...
    # first prepare data
    snp.dist.df %<>%
      dplyr::mutate(WithinStrain = (Distances<(strain_intercept+strain_slope*SNPDistances))) %>%
  ...
  for (r in rec_levels) {
    for (s in select_levels) {
      ...
      for (i in unique(sim.set$Replicate)) {
        sim_vals<-as.numeric(unlist(sim.set[sim.set$Replicate==i,colnames(sim.set)==vals_colname]))
        ktest<-suppressWarnings(ks.test(actual,sim_vals))
...
ggsave(ma.tree.plot,file="~/Documents/evoMLNFDS/rec_v_select/zero_time/figures/revised.ma_tree.png",width=20,height=20,units = "cm")
...
permuted_prefixes<-c("/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/randomised_inputs/permuted_outputs/rec_v_select_no_mig/rec_v_select_no_mig",
...
# analysis with markers
subset_prefixes<-c("/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/subset_inputs/rec_v_select_no_mig/rec_v_select_no_mig",
                     "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/subset_inputs/rec_v_no_select_no_mig/rec_v_no_select_no_mig",
                     "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/subset_inputs/no_rec_v_select_no_mig/no_rec_v_select_no_mig",
                     ...
                     "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/subset_inputs/sym_rec_v_select_no_mig/sym_rec_v_select_no_mig",
...
weak_selection_time_series_prefixes<-c("/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/zero_time/time_series/rec_v_weak_select_no_mig",
                                       "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/zero_time/time_series/rec_v_no_select_no_mig",
                                       "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/zero_time/time_series/no_rec_v_weak_select_no_mig",
                                       "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/zero_time/time_series/no_rec_v_no_select_no_mig",
                                       "/Users/nicholascroucher/Documents/evoMLNFDS/rec_v_select/zero_time/time_series/sym_rec_v_weak_select_no_mig",
...
# accessory data
acc.data<-ma.data[,6:ncol(ma.data)]
